...
причинности с механизмом. Сначала сверься с банком сигналов (`rg.py signals`):
подтверждённые и `reusable` сигналы из закрытых гипотез — легитимные блоки
новой цепи (ссылайся на hid-источник в `independent_of`/evidence); утверждения,
опровергнутые тестом, повторять нельзя.
