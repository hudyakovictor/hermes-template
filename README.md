...
## Mini App (пульт в телефоне)

`miniapp/` — Telegram Mini App поверх того же контура: кольцо VRAM и бюджет,
очередь гипотез по PPI со свайп-действиями, живые loss-кривые по сидам,
чат и рынок прогнозов экипажа, вердикты «обещали vs получили», пошаговая
подача идеи с дедупликацией. Без внешних зависимостей: чистые HTML/CSS/JS +
`serve.py` на stdlib.

```bash
python3 miniapp/serve.py --port 8899   # дальше HTTPS-туннель и BotFather → /newapp
```

Дизайн-спецификация и карта разделов — в [docs/DESIGN-MINIAPP.md](docs/DESIGN-MINIAPP.md),
запуск и подключение — в [miniapp/README.md](miniapp/README.md).
