# Аудит функционала: 35 анализов → топ-20 ошибок → исправления

Дата: 2026-08-30 · инструмент: `python tools/rg.py audit` (или `python tools/audit.py run`)

## Метод

35 детерминированных анализов гоняют **реальный код** — библиотечные вызовы на
...
честный: до исправлений он находил ошибки, после — 35/35 зелёные.
...
```bash
python tools/rg.py audit            # 35 анализов + отчёт reports/audit-<дата>.md
