...
| **Скиллы** | `skills/<name>/SKILL.md`, 19 шт. | Каждый скилл — слеш-команда и сжатая инструкция: контекст грузится только когда нужен; `/bottom` подключает hybrid search layer |
| **Комплекты скиллов** | `skill-bundles/research-os.yaml` | Одна сущность вместо 19 разрозненных файлов при установке и обновлении |
...
| **Канбан** | `tools/board.py sync` | Визуальная картина для человека; одностороннее зеркало, истина остаётся в SQLite |
