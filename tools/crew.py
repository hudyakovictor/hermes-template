...
AGENTS: dict[str, dict] = {
    ...
    "morg":    {"name": "Морг",     "zone": "kill-stage: 8 проверок, контраргументы"},
...
# тоне» — тролль, наивняк, сухарь. В сцене любой может съязвить, любой —
# поддержать; персональные тоны проявляются в перекрёстных репликах.
...
SCENES: dict[str, list[tuple[str, str, list[str]]]] = {
    "hypo_new": [
        ...
        ("morg", "work", [
            "прогноз {forecast}, красиво. теперь вопрос всем — чем это объясняется проще. жду.",
            "всем слышно? записал {forecast}. теперь это обещание, и оно у всех на виду.",
        ]),
        ("shef", "work", [
    ...
    "signal_recall": [
        ("hronik", "work", [
            "стоп, это уже было: {claim} — из {hid}. сигнал живой, гипотеза нет.",
            "память подсказывает: {hid} падал на похожем, но сигнал {outcome}. можно переиспользовать.",
        ]),
        ("stazhor", "work", [
            "запишу связь в карточку: новая идея + старый сигнал из {hid}.",
        ]),
    ],
    "customer_lead": [
        ("shef", "work", [
        ...
        ("morg", "work", [
            "пока вы решаете, взлетит или нет: прошлый «гарантированный» лид умер за десять минут. чек-лист тот же.",
            "коллеги, красиво спорите. я карточку перечитал: половина секций пустая.",
        ]),
        ("stazhor", "work", [
            "а вдруг эта взлетит?",
        ]),
    ...
    "gate_pass": [
        ("morg", "work", [
            "{hid} прошла все kill-проверки (8/8). не привыкайте, это аномалия.",
        ]),
        ("gayka", "work", [
    ...
    "verdict_confirmed": [
        ("morg", "work", [
            "{hid} подтверждена. факт {actual} при прогнозе {forecast}, отклонение {dev}. редко скажу — красиво.",
            "{bets_result}",
        ]),
        ("gayka", "work", [
    ...
    "queue_empty": [
        ...
        ("morg", "work", [
            "вопрос ко всем: когда в очередь вернётся что-то живое?",
        ]),
        ("stazhor", "work", [
    ...
    "weekly": [
        ...
        ("morg", "work", [
            "оптимизм — это прогноз без контроля. всем, кто ставил на плюс.",
        ]),
        ("hronik", "work", [
    ...
    "budget_burn": [
        ...
        ("morg", "work", [
            "лимит — милосердие по отношению к гипотезам. и к нам всем.",
        ]),
    ],
    "mode_change": [
        ("shef", "work", [
    ...
    "review_dup_signals": [
        ...
        ("krot", "work", [
            "бро Скиф, у тебя фильтр дублей по старому кэшу работает. перезапусти.",
        ]),
    ],
    "review_budget_pace": [
        ("shef", "work", [
    ...
    "idea_intake": [
        ...
        ("skif", "work", [
            "в тексте {signals_est} намёков на источники. пока это виба, не сигнал.",
            "если там ссылки — посмотрю каждый. если слова — посмотрю скептически.",
        ]),
        ("morg", "work", [
            "вопрос на разборе один: кто заплатит. ставлю его первым.",
        ]),
    ],
    "idea_review": [
        ("krot", "work", [
            "разбор {iid}: перспективность {pi}, приоритет {ppi}. цифры против энтузиазма, как всегда.",
            "{iid}: {note}. ориентировка — {pi}. дальше пороги, не вкусы.",
        ]),
        ("skif", "work", [
            "источники: {signals}. три независимых — минимум, меньше в очередь не ставим.",
        ]),
        ("morg", "work", [
            "деньги: {money}. ниже половины — считаем, что покупателя нет.",
        ]),
        ("shef", "work", [
            "пороги всем известны: сигналов ≥ 3, перспективность ≥ 0.40. по цифрам и решаем.",
        ]),
    ],
    "idea_queued": [
        ("krot", "work", [
            "{iid} → очередь: {hid}. прогноз {forecast}, приоритет {ppi}.",
        ]),
        ("gayka", "work", [
            "{bets_line}",
        ]),
        ("shef", "work", [
            "{hid} в работе. дальше гейт из 8 проверок — без исключений.",
        ]),
    ],
    "idea_rejected": [
        ("morg", "work", [
            "{iid}: {reason}. фиксирую в лог неэффективных.",
        ]),
        ("shef", "work", [
            "отклонено — не похоронено: принесёте новый факт, вернёмся.",
        ]),
        ("krot", "work", [
            "записал. кто скинет дубль — узнает об этом в ту же секунду.",
        ]),
    ],
    "idea_dup": [
        ("krot", "work", [
            "это уже рассматривали: {dup_id} — {dup_verdict}. похожесть {score}.",
        ]),
        ("morg", "work", [
            "{dup_why}. новые данные есть? нет — ответ тот же.",
        ]),
        ("shef", "work", [
            "дубль дважды не разбираем. поднимите факт — тогда разговор другой.",
        ]),
    ],
    "demand_block": [
        ("shef", "work", [
    ...
    "commercial_dead_end": [
        ...
        ("hronik", "work", [
            "money {money}, покупателя нет. красиво, бесполезно.",
        ]),
        ("krot", "work", [
            "в следующий раз — сначала «кому продадим», потом L0.",
        ]),
    ],
    "agi_day": [
        ("hronik", "customer", [
...
JOKES: list[dict] = [
    ...
    {"text": "запустили, упало. починили, запустили, снова упало.",
...
DISPUTES: list[dict] = [
    {   # проверка гипотезы на прочность
        ...
        "lines": [
            ("morg", "стоп. {hid}. чем это объясняется проще. lr, утечка, контроль."),
...
def render_scene(event: str, ctx: dict, rng: random.Random,
    ...
    for agent, kind, variants in blocks[:limit]:
        ...
        text = bro_prefix(text, agent, rng)
        text = decorate_line(text, agent, rng)
        lines.append({"agent": agent, "kind": kind, "text": text,
                      "event": event})
    return lines
...
# --------------------------------------------------------------- пересечения
# Мини-диалоги «в адрес коллеги»: пока одни ругаются между собой, Морг говорит
# с чатом целиком, Васёк просит помощи и получает ответ, в котором не понять —
# помощь это или сарказм, Аналитег троллит с «бро» в начале. Пары выбираются
# случайно; приоритет у агентов, не занятых в текущей сцене (обычно Морг
# и iВасёк) — так все пересекаются со всеми в разных комбинациях.
BANTER_EXCHANGES: list[list[tuple[str, list[str]]]] = [
    # Морг — в самый разгар чужого спора, обращается ко всем
    [("morg", [
        "пока вы тут ругаетесь, чей сигнал сильнее — контрольного условия в карточке всё ещё нет.",
        "напомню всем: прогноз без коридора — не прогноз, а настроение.",
        "вы договоритесь до чего угодно, кроме контрольного условия. я подожду.",
    ]), ("stazhor", [
        "морг, а если она и правда живая? я просто спрашиваю, все же спорят.",
        "а мне кажется всем нравится спорить больше, чем проверять.",
    ])],
    # Васёк просит помощи — ответ Гайки: помощь или сарказм, не понять
    [("stazhor", [
        "гайка, а как правильно чекпойнт ставить? в прошлый раз у меня всё утекло.",
        "гайка, помоги с карточкой? ты же умеешь заполнять все секции…",
    ]), ("gayka", [
        "васёк, смотри выше — я показывала дважды. но давай ещё, мне не жалко.",
        "конечно, помогу. открой карточку. это та, что с твоим именем сверху.",
    ])],
    # Аналитег троллит Гайку — Гайка отвечает по делу, сухо
    [("krot", [
        "бро Гайка, твои seeds снова пляшут. или это дискотека?",
        "бро, ну и кривая. ты уверена, что это обучение, а не кардиограмма?",
    ]), ("gayka", [
        "аналитег, я скрипт запускаю, а не мнения. потерпи до метрик.",
        "когда научишься считать сам — приходи обсуждать seeds.",
    ])],
    # Васёк наивно влезает к Аналитегу — сарказм в ответ
    [("stazhor", [
        "аналитег, а почему у сигнала только два источника? он же слабый.",
        "аналитег, а ты правда считал, или как обычно — по ощущениям?",
    ]), ("krot", [
        "бро Васёк, ты сегодня неожиданно прав. пугает.",
        "считал, бро. ощущения — это когда говоришь ты.",
    ])],
    # Скиф мягко проезжается — Аналитег отвечает в своём стиле
    [("skif", [
        "аналитег, ты сигналы складываешь или пересказываешь их мне?",
        "если бы споры двигали науку, мы бы уже AGI собрали.",
    ]), ("krot", [
        "бро, добыча — полдела. вторая половина — меня послушать.",
        "складываю, бро. тебе не рассказываю — ты расстроишься.",
    ])],
    # Хроник вспоминает прецедент — всем
    [("hronik", [
        "это уже было в марте. тогда тоже все ругались. гипотеза умерла, выводы остались.",
        "по архиву: такие споры кончаются вердиктом, а не согласием. проверим.",
    ])],
    # Гайка изредка даёт сдачи М orgу
    [("gayka", [
        "морг, ты в курсе, что твой чек-лист длиннее моего скрипта?",
    ]), ("morg", [
        "в курсе. чек-лист дешевле, чем GPU-час. считай это комплиментом скрипту.",
    ])],
    # одиночные подколы — редкие, чтобы не приедались
    [("krot", [
        "бро Васёк, карточка снова наполовину. ты их коллекционируешь?",
    ])],
    [("morg", [
        "всем, кто ставит на подтверждение: я перечитал, почему нет. ставьте смелее.",
    ])],
]

# Эмодзи и «))»/«(())» — изредка и по делу: вероятность по умолчанию 30%,
# у скептика (Морга) и Bossа — реже. Выбор знака — от тона реплики.
EMOJI_POS = (" ))", " 🔥", " ✅")
EMOJI_NEG = (" ((", " 💀", " 🫠")
EMOJI_ANY = (" ))", " 👀", " 🫡")
DECOR_PROB = {"shef": 0.0, "morg": 0.08, "hronik": 0.18}
_EMOJI_TAIL = (")", "!", "?", ".", "…")


def decorate_line(text: str, agent: str, rng: random.Random) -> str:
    """Изредка добавить смайл — только если реплика это позволяет."""
    if rng.random() >= DECOR_PROB.get(agent, 0.30):
        return text
    low = text.lower()
    stripped = text.rstrip()
    # уже украшено (эмодзи, скобки) или стоит многоточие — не добавляем
    if stripped and not stripped[-1].isalnum() and not stripped.endswith((",", ";")):
        return text
    if any(w in low for w in ("мусор", "умер", "мимо", "упал", "слаб", "косяк",
                              "хоронил", "утекло", "пуст")):
        tail = rng.choice(EMOJI_NEG)
    elif any(w in low for w in ("прошла", "чисто", "готов", "подтвержд",
                                "красиво", "взлетит", "живая", "победа")):
        tail = rng.choice(EMOJI_POS)
    else:
        tail = rng.choice(EMOJI_ANY)
    return text.rstrip() + tail


def bro_prefix(text: str, agent: str, rng: random.Random, prob: float = 0.30) -> str:
    """«бро» — фирменное начало Аналитега: изредка, и только в начале."""
    if agent != "krot" or text.lower().startswith("бро"):
        return text
    if rng.random() >= prob:
        return text
    return "бро, " + (text[0].lower() + text[1:] if text[:1].isupper() else text)


# Микро-факторы общения (20): критерии и мелочи, которые любой агент мог
# забыть, а другой — случайно вспомнить, глядя на чужое сообщение. В разных
# комбинациях по контексту они и задают живую случайность диалога; 70% таких
# вставок — конструктив (напоминание, поправка цифры, признание ошибки).
MICRO_FACTORS: tuple[str, ...] = (
    "коридор прогноза зафиксирован до запуска",
    "контрольное условие, где эффект обязан исчезнуть",
    "разброс по seeds, а не один прогон",
    "утечка train-test исключена",
    "простое объяснение проверено: lr, init, batch",
    "публикационный gap сверен по двум базам",
    "покупатель назван, а не виба",
    "метрика читаема до полного обучения",
    "PASS/FAIL числами до факта",
    "оценка GPU-часов не занижена",
    "спрос-сигналы свежие, не прошлогодние",
    "дубль сигнала отфильтрован",
    "p_repro не оптимистичнее base rate",
    "масштаб честный: сначала Tiny, потом большая",
    "план на вытеснение есть, чекпойнт пишется",
    "калибровочный сдвиг учтён в прогнозе",
    "абляция по одному фактору за раз",
    "веса PI пересчитаны после рекалибровки",
    "источник независимый, не круговая ссылка",
    "экономия посчитана в GPU-часах, не в процентах",
)

RECALL_LINES = {
    "remind": (          # вспомнил критерий — напоминает говорившему
        "кстати, проверь: {factor}. я чуть не забыл в прошлый раз.",
        "глянь ещё раз на {factor}. мне не даёт покоя.",
        "удержи мысль: {factor}. это дешевле, чем переделывать."),
    "fix": (             # поправка цифры/формулировки
        "цифру подправь: в карточке округлили, по базе точнее.",
        "формулировка неточная: зафиксировано до запуска, не после.",
        "тут знак спутан: экономия в GPU-часах, не в процентах."),
    "admit": (           # признание ошибки / отзыв своего вывода
        "признаю, мой вывод был по одному сиду. откатываю, пересчитаю.",
        "я был неправ вчера: прогноз ставил по вибе. исправлено.",
        "отзываю своё замечание — перепроверил, там всё чисто."),
}


def render_recall(rng: random.Random, ctx: dict) -> dict | None:
    """Одна микро-вставка: 70% конструктив (напомнить/поправить/признать),
    30% — нейтральный вопрос по делу. Автор — случайный, кроме Boss."""
    agent = rng.choice([a for a in AGENTS if a != "shef"])
    if rng.random() < 0.7:
        kind = rng.choices(("remind", "fix", "admit"), weights=(50, 30, 20))[0]
        text = rng.choice(RECALL_LINES[kind])
        if "{factor}" in text:
            text = text.format(factor=rng.choice(MICRO_FACTORS))
        return {"agent": agent, "kind": "work", "text": text,
                "event": "recall", "recall_kind": kind}
    neutral = rng.choice((
        "а по регламенту это через гейт или сразу?",
        "дай ссылку на источник, хочу сам глянуть.",
        "сколько это стоит в часах, если я не прав?",
    ))
    return {"agent": agent, "kind": "work", "text": neutral,
            "event": "recall", "recall_kind": "neutral"}


def render_banter(ctx: dict, rng: random.Random, spoken: set[str],
                  room: int) -> list[dict]:
    """Одно пересечение личностей (1-2 реплики) в конец сцены.

    Приоритет — обменам, где никто ещё не говорил: не выступившие в сцене
    (чаще всего Морг и iВасёк) получают тройной вес.
    """
    if room <= 0 or not BANTER_EXCHANGES:
        return []
    weights = []
    for ex in BANTER_EXCHANGES:
        speakers = {a for a, _ in ex}
        if speakers & spoken:
            weights.append(0.3 if speakers <= spoken else 1.0)
        else:
            weights.append(3.0)
    exchange = rng.choices(BANTER_EXCHANGES, weights=weights, k=1)[0]
    out: list[dict] = []
    for idx, (agent, templates) in enumerate(exchange[:max(1, room)]):
        if idx == 1 and rng.random() > 0.7:
            break  # ответ иногда не приходит: вопрос повис — тоже жизнь
        text = _fmt(rng.choice(templates), ctx)
        out.append({"agent": agent, "kind": "work", "text": text,
                    "event": "banter"})
    return out
...
def emit(event: str, ctx: dict | None = None, conn: sqlite3.Connection | None = None,
    ...
    # пересечения личностей: изредка в конец сцены влетает мини-диалог пары —
    # приоритет у тех, кто в сцене ещё не говорил (обычно Морг и iВасёк).
    # Как и спор, бантер замещает хвост сцены, а не удлиняет её: контракт
    # «1-5 реплик без спора» держится и здесь.
    if event not in ("agi_day", "joke") and rng.random() < 0.55:
        cap = base_limit + (2 if any(l.get("dispute_id") for l in lines) else 0)
        want = 2 if rng.random() < 0.45 else 1
        room = cap - len(lines)
        if room > 0:
            extra = render_banter(ctx, rng, {l["agent"] for l in lines},
                                  min(want, room))
            need = len(lines) + len(extra) - cap
            if need > 0:
                lines = lines[:max(1, len(lines) - need)]
            lines += extra

    # микро-фактор: один из агентов случайно вспомнил критерий / поправил
    # цифру / признал ошибку — 70% таких вставок делают работу лучше
    cap = base_limit + (2 if any(l.get("dispute_id") for l in lines) else 0)
    if event not in ("agi_day", "joke") and len(lines) < cap \
            and rng.random() < 0.22:
        recall = render_recall(rng, ctx)
        if recall:
            lines.append(recall)
